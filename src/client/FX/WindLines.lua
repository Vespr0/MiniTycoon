...
function module:Init(Settings)
	...
	module.Direction = workspace.GlobalWind
	...
	if module.MatchWorkspaceWindConnection then
		module.MatchWorkspaceWindConnection:Disconnect()
		module.MatchWorkspaceWindConnection = nil
	end
	...
	-- Connect direction with GlobalWind
	if Settings.MatchWorkspaceWind ~= false then -- Default to true unless explicitly disabled
		module.Direction = workspace.GlobalWind
		module.MatchWorkspaceWindConnection = workspace:GetPropertyChangedSignal("GlobalWind"):Connect(function()
			module.Direction = workspace.GlobalWind
		end)
	end
