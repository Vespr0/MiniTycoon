...
-- Debug flags --
local DISABLE_GRID_SNAPPING = false -- Set to true to disable grid snapping
local DISABLE_LERPING = false -- Set to true to disable position lerping
...
function ClientPlacement.StartPlacing(itemName, model, moving)
	...
	model.Parent = Nodes
	model:SetAttribute("ItemName", itemName) -- Set ItemName for terrain validation
	...
	trove:Connect(RunService.RenderStepped, function(deltaTime)
		...
		local targetCFrame = DISABLE_LERPING and goalCFrame or modelRoot.CFrame:Lerp(goalCFrame, deltaTime / lerpTime)
		model:PivotTo(targetCFrame)

		validPlacement = PlacementUtility.isPlacementValid(plot, model, overlapParams, config)
