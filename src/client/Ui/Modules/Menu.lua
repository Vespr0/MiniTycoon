...
-- Require all modules at startup
local modules = {
	Storage = require(UiModules.Storage),
	Shop = require(UiModules.Shop),
	ControlPanel = require(UiModules.ControlPanel),
	Index = require(UiModules.Index),
}
...
function Menu.Setup()
	...
	for _, button in MainFrame:GetChildren() do
		...
		-- Register alert for this button with AlertsManager
		local moduleName = button.Name
		AlertsManager.RegisterAlert("menu_" .. moduleName, button.Icon)
	end
